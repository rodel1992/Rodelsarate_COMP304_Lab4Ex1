...
public class DatabaseManager extends SQLiteOpenHelper {
    ...
    void addRecord(ContentValues values, String tableName, String fields[],String record[]) {
        ...
        for (int i=1;i<record.length;i++) {
            if (i == 3 || i == 5) {
                values.put(fields[i], Integer.parseInt(record[i]));
            } else {
...
//    void addRecord(ContentValues values, String tableName, String fields[],ArrayList record) {
//        SQLiteDatabase db = this.getWritableDatabase();
//
//        for (int i=0;i<record.size();i++)
//            values.put(fields[i+1], record.get(i).toString());
//        // Insert the row
//        db.insert(tableName, null, values);
//        db.close(); //close database connection
//    }
